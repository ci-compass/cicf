...
It is easy to learn, has automatic memory management, and can be extended easily using packages.
...
Start it with the `python3` command.

```
dbrower@crc-don ~/c/cicf (week02-update)> python3
...
(Not forever, though. You can exit by typing `Ctrl-D` or the command `exit`.)
...
The above makes a variable `x` and sets it equal to 2 to the 80th power.
We then compute the base 2 logarithm on `x`, which is 80.
We add `x` to itself, note that this does not change x.
We divide `x` by `x+1`.
Surprisingly, the answer is 1!
This is because the division is represented as a _floating point_ number, and the closest floating point number to
the mathematically precise number is 1.0.
The ".0" part is what tells you it is floating point when printing it out.
We can verify the type by using the function `type()`.
The underscore `_` means "the previous result".
