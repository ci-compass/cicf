...
#
# Additional hackage (for clarity) by Erik.L.Scott@gmail.com
#
...
Entered the exists() method. Here we'll see if the table
exists and we'll return a boolean (true if it exists)
...
Entered the create_table() method. This will create the table (hence
the name) and should only be called if the table doesn't exist. That's
why we call exists() first to check and see.
...
Entered the list_tables() method. As expected, this generates a
list of all the tables. They are returned to the calling
function as a Python list.
...
This is the write_batch() method. It's a faster way to write a
whole bunch of records into Dynamo because it makes
fewer round-trip protocol requests.
...
This is the add_movie() method. I bet you can guess by
now what it does.
...
This is get_movie(). It calls "get_item()" from the boto3 package
(Amazon's convenience library for building and running protocol
requests). In this case, because it's reading a table of movies,
...
update_movie() is another one of those methods that does what
it says on the tin. Note how the UpdateExpression statement is
built and how it works with ExpressionAttributeValues. In a
bit of syntax reminiscent of a relational database's stored
procedures, the statement is built using placeholders for the
values and the attributes are then applied to make a full
command.
...
Now we are in the query_movies() method. This is very straightforward.
...
This is the scan_movies() method. As the name implies, this does
a "full table scan", or in other words it reads every last item
in the table looking for ones that match the search criteria.
That's one of the problems with NoSQL databases in general:
Searches that you positively know will only return one result
can be executed very, very quickly. But as soon as the query gets
even slightly complicated, performance goes down the...
performance gets very bad.
    ...
    def delete_movie(self, title, year):
        ...
        This is the delete_movie() method. It's pretty darn simple,
        as one would expect. Note that "year" and "title" are both
        required, and together they are unique throughout the table
        (DynamoDB enforces this).
...
This is the delete_table() method. All it does is call the
boto3 package's delete method for Dynamo.
...
This is the run_scenario() method. It handles calling the various functions
above after it asks some questions at each step. I'm sorry it's such a long
function and I assure you if the author had done this in my Python class
they'd be certain to lose a few points.
